...
app.post('/telegram-webhook', async (req, res) => { 
    ...
    if (text === '/start' || text === '/menu' || text === '/help') { 
        ...
        const menu = `*\\|👋\\|*\\ Estos son los comandos disponibles:\\\n\\\n` + 
                     ...
                     `\n*\\|/uptimestatus\\|* \\- Verifica el estado público de UptimeRobot\\.`; // Agregado el nuevo comando al menú
    ...
    // NUEVO COMANDO: /uptimestatus
    if (text === '/uptimestatus') {
        if (process.env.UPTIMEROBOT_STATUS_PAGE_URL) {
            const statusMessage = `\\|🟢\\| Puedes ver el estado público de UptimeRobot aquí: [Página de Estado](${escapeMarkdown(process.env.UPTIMEROBOT_STATUS_PAGE_URL)})`;
            await sendTelegramMessage(chatId, statusMessage);
        } else {
            await sendTelegramMessage(chatId, '\\|⚠️\\| La URL de la página de estado de UptimeRobot no está configurada\\. Contacta al administrador\\.');
        }
        return res.sendStatus(200);
    }
